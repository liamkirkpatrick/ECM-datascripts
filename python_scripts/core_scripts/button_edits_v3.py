...
data = [
    ...
    #if row['core'] == 'alhic2401' and int(row['section']) < 10 and row['ACorDC'] == 'AC'
    #if row['core'] == 'alhic2416'
    #if row['core'] == 'alhic2401' and row['ACorDC'] == 'AC'
    if row['core'] == 'alhic2401' and row['ACorDC'] == 'DC'
]
...
def process_ecm(d):
    ...
    def on_key(event):
        ...
        if key == 'd':
            ...
            # Save and move on
            df.to_csv(path_to_data + d.core + '/' + fname, index=False)
            print(f"    Saved {fname}")
            plt.close(fig)
